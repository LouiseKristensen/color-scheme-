...
document.querySelector("#colorGenerator").addEventListener("submit", function(e){
    e.preventDefault(); 

    const color = document.querySelector("#favcolor").value.substring(1); 
    const mode = document.querySelector("#colorcombos").value; 
        ...
        .then(data => {
            ...
            data.colors.forEach(colorobj =>{
                ...
                p.addEventListener("click", () => {
                    navigator.clipboard.writeText(colorobj.hex.value)
                        .then(() => alert(`${colorobj.hex.value} copied!`))
